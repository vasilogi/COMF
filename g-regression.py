...
output_path = os.path.join(output_path,'g-regression')
...
for i_file in range(len(csv_files)):
	...
	for i_model in range(len(modelnames)):
		...
		gexp           = [model.g(i) for i in a]         # Define the g rate based on the experimental conversion
		params, extras = curve_fit(fit,t,gexp,bounds=(0.00001,10.0))
		k              = params[0]                       # The k parameter
		gcal           = [fit(i,k) for i in t]           # The simulated g values given the k calculation
		gexmean        = np.mean(gexp, dtype=np.float64) # The mean of the experimental values
		...
		for i in range(len(gcal)):
			SSres += (gexp[i] - gcal[i])*(gexp[i] - gcal[i]) # Residual sum of squares
			SStot += (gexp[i] - gexmean)*(gexp[i] - gexmean) # Total sum of squares
		...
		filename = 'gcalc_vs_t_'+model.module_name+'_model.csv'
		if not os.path.isfile(os.path.join(spec_model_path,filename)):
			filepath = os.path.join(spec_model_path,filename)
			tmp = np.transpose([gcal,t])
			np.savetxt(filepath, tmp, delimiter=",")
			# Add header in the csv file
			with open(filepath,newline='') as f:
				r = csv.reader(f)
